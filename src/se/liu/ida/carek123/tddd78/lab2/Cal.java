...
public class Cal
{
    ...
    public void book(int year, String month, int day, int startHour, int startMinute, int endHour, int endMinute,
		     String subject)
    {
	if (year > 1970 && 0 <= startHour && startHour <= 23 && 0 <= endHour && endHour <= 23 && 0 <= startMinute &&
	    startMinute <= 23 && 0 <= endMinute && endMinute <= 23 && day == Month.getMonthDays(month) &&
	    Month.getMonthDays(month) != -1) {
